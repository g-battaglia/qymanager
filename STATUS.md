...
**Ultimo aggiornamento**: 2026-04-23 (Session 33a — **Web editor merge-capture** + voice-byte triangulation empirica. Endpoint `POST /api/devices/{id}/merge-capture` + pulsante "Merge XG capture" nella UI replicano `qymanager merge` dal browser: voci reali (Bank MSB/LSB/Program, Vol/Pan/Rev/Cho) popolate end-to-end verificate su AMB#01 ground truth. Fix `parse_xg_bulk_to_udm`: ora scansiona anche channel events con running-status (XG PARM OUT emette Bank/Prog come `Bn 00 / Bn 20 / Cn`, non come XG Param Change). `midi_tools/build_signature_db.py` + DB esteso 23 → 28 entry. **Triangulation**: su 16 data point AMB01+STYLE2, voice encoding è un pacchetto ~20 byte (offset 28-47) che contiene ROM index + voice override parameters — non un mapping 1:1 byte→program. Signature DB oltre 10-byte non dis-ambigua. **Voice ROM table resta opaca dal bulk alone** (richiede firmware dump); workaround merge-capture ora fruibile sia da CLI che da UI. **494 test passed, 3 skipped**. 4 nuovi test device merge-capture, +3 test analysis.)
