...
**Ultimo aggiornamento**: 2026-04-27 (Session 34 — **XG Multi Part 100% copertura**: tutti i ~70 parametri XG Multi Part (AL 0x00-0x6E) ora mappati in address_map, schema, xg_bulk, syx_analyzer. **5 bug critici fixati**: (1) filter/EG AL off-by-3 in address_map (cutoff era 0x15, corretto a 0x18); (2) entry duplicata `same_note_number_key` rimossa, part_mode/note_shift/detune shiftati; (3) drum setup filter/EG AL corretti (da 0x14-0x18 a 0x0B-0x0F); (4) syx_analyzer AL=0x23/0x24 rinominati da filter_cutoff/resonance a bend_pitch/bend_filter; (5) xg_bulk _apply_multi_part esteso da 11 a 70+ handler AL. **~60 nuovi campi MultiPart**: vibrato, MW control, bend extended, 17 rx switches, 12 scale tuning, CAT/PAT/AC1/AC2 control, portamento, pitch EG, velocity limits. **Detune 2-byte**: emit MSB+LSB in ops.py. **185 nuovi test**: address_map spec cross-reference, parse round-trip per ogni AL, set→emit→parse round-trip, schema validation, completeness coverage. **691 test passed, 3 skipped**.)
...
| Estrazione SysEx: tutti i parametri XG documentati | **Production-ready** | **100%** (~70 Multi Part params AL 0x00-0x6E, System, Effects, Drum Setup, AH=0x05 name directory) |
...
- **Test suite**: **691 passed, 3 skipped** (inclusi 9 property + hardware-gated + 185 Multi Part completeness/roundtrip). Full run: `uv run pytest -q` in ~15s.
