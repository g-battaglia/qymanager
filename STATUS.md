...
**Ultimo aggiornamento**: 2026-04-23 (Session 33a→d — **RE SysEx saturato end-to-end**: tutto quello che è decodificabile software-only dal bulk QY70 ora fluisce nell'UDM, nell'API `/syx-analysis` e nella UI web, inclusi i note events, il MIDI export, voice NN-fuzzy matching, e propagation XG embedded (voices, system, effects, drum setup). Highlights: (1) `merge-capture` endpoint + pulsante "Merge XG capture" (voci reali da XG capture JSON/.syx) + `_raw_passthrough` append così `/syx-analysis` re-legge lo stream mergiato; (2) `parse_xg_bulk_to_udm` ora decodifica anche channel events con running-status (`Bn 00/20 CC7/10/91/93/94 + Cn`) + Master Tune (4-nibble) + Master Attenuator + Drum Setup per-note; (3) `parse_syx_to_udm` via `SyxAnalyzer` → tempo reale, canali PATT OUT 9-16, sezioni Intro/FillAB/FillBA/Ending in `SectionName` enum, voci DB-resolved in `Voice()`; (4) **decoder sparse R=9×(i+1) wired nella pipeline** con guard-rail plausibility ≥60%: MR. Vain → 97 NOTE_ON events, Summer → 53, factory dense skippati correttamente (nessun ghost note); (5) GM drum names (Kick 1 / Snare 1 / Closed Hat / ...); (6) SMF export sintetizza Song da `phrases_user` → file `.mid` 6-track ascoltabile in DAW; (7) `/syx-analysis` espone xg_system (master tune/vol/attenuator/transpose), pattern_directory (slot names AH=0x05), drum_kits (per-note GM name + level); (8) UI mostra tutto senza nested card innestate; (9) signature DB 23 → 29 entries con `--manual` mode. **Triangulation empirica** su 16 data point: voice encoding = pacchetto ~20 byte (offset 28-47), non mapping 1:1 byte→program. **Gap residuo oggettivo**: (a) voice ROM table → firmware dump QY70; (b) decoder dense → Session 20 structural impossibility; (c) time_sig byte → *una* capture 3/4 o 6/8; (d) bar_count byte → *una* capture multi-bar di lunghezza nota. **503 test passed, 3 skipped**. 16 commit pushati in `a6cf7d6..e19cd6e`.)
...
| Conversione QY70 → QY700 (Pipeline A SysEx decode) | **Near-complete** | **98%** (sparse 100%, dense bitstream 100%, dense-user 85% semantic ceiling, **voice 100% via live XG capture o via embedded XG block, 80-100% user-sparse via DB+NN class-aware, 100% full-backup via embedded XG propagation**, playback verify funzionante, **phrases user decodificati via R=9 ora in UDM.phrases_user**) |
